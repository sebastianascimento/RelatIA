...
def extract_text_from_file(file_content, filename):
    ...
    if extension in ['.txt', '.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.c', '.cpp', '.html', '.css']:
        try:
...
def analyze_file(file_content, filename):
    ...
    if text_content.startswith("Unsupported file type"):
        ...
        text_content = f"// Conteúdo do arquivo {filename} (não foi possível extrair o texto)"
    ...
    def run_agent_with_capture(agent, task_creator, agent_name):
        ...
        try:
            ...
            with redirect_stdout(output_capture):
                crew = Crew(agents=[agent], tasks=[task_creator(agent, text_content)], verbose=2)
            ...
            if matches and len(matches[-1].strip()) > 10:
                ...
                print(f"DEBUG: Extraído resultado de {len(extracted_result)} chars para {agent_name}")
    ...
    try:
        ...
        structure_result = run_agent_with_capture(
            structure_agent, 
            task_factory.create_structure_analysis_task,
            "estrutura"
        )
        ...
        security_result = run_agent_with_capture(
            security_agent, 
            task_factory.create_security_analysis_task,
            "segurança"
        )
        ...
        quality_result = run_agent_with_capture(
            quality_agent, 
            task_factory.create_quality_analysis_task,
            "qualidade"
        )
    ...
    for agent, content in analysis_results.items():
        ...
        if content is None or len(content) < 10:
            ...
            analysis_results[agent] = f"Análise para {agent} não foi completada adequadamente. Possível erro de processamento."
